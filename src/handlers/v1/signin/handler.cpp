...
namespace handlers::authorization_v1_signin {
...
userver::formats::json::Value Handler::HandleRequestJsonThrow(
  ...
  switch (request.GetMethod()) {
    case userver::server::http::HttpMethod::kPost:
      return SignIn(request, request_json);
    default:
      throw ClientError(ExternalBody{std::format("method-not-allowed")});
  }
}

userver::formats::json::Value Handler::SignIn(
    const userver::server::http::HttpRequest& request,
    const userver::formats::json::Value& request_json) const {
  try {
    const auto& user_phone_number =
        request_json["phoneNumber"].As<std::string>();
    const auto& access_token = _auth_manager.SignIn(user_phone_number);
